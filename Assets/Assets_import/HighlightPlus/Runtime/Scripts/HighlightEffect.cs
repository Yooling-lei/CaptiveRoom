...
namespace HighlightPlus
{
    ...
    public delegate void FullScreenBlitMethod(CommandBuffer cmd, RenderTargetIdentifier source,
    ...
    public partial class HighlightEffect : MonoBehaviour
    {
        ...
        [GradientUsage(hdr: true, ColorSpace.Linear)]
        ...
        [SerializeField, HideInInspector] ModelMaterials[] rms;
        [SerializeField, HideInInspector] int rmsCount;
        ...
        [NonSerialized] public static List<HighlightEffect> effects = new List<HighlightEffect>();
        ...
        readonly Dictionary<Camera, PerCameraOcclusionData> perCameraOcclusionData =
            new Dictionary<Camera, PerCameraOcclusionData>();
        ...
        public CommandBuffer GetCommandBuffer(Camera cam, RenderTargetIdentifier colorAttachmentBuffer,
            RenderTargetIdentifier depthAttachmentBuffer, FullScreenBlitMethod fullScreenBlit, bool clearStencil)
        {
        ...
        void BuildCommandBuffer(Camera cam, bool clearStencil)
        {
            ...
            // 物体被遮挡时穿透效果
            bool seeThroughReal = seeThroughIntensity > 0 && (seeThrough == SeeThroughMode.AlwaysWhenOccluded ||
            ...
            bool renderMaskOnTop = _highlighted && ((outline > 0 && smoothOutlineVisibility != Visibility.Normal) ||
                        ...
                        for (int glowPass = 0; glowPass < glowPasses.Length; glowPass++)
                        {
                            if (glowQuality.UsesMultipleOffsets())
                            {
                                for (int o = glowOffsetsMin; o <= glowOffsetsMax; o++)
                                {
                                    ...
                                    Vector4 dataGlow = new Vector4(fade * glowReal * glowPasses[glowPass].alpha,
                ...
                if (targetFX)
                {
                    ...
                    if (fadeOut > 0 && !(targetEffectRendered && usesTarget))
                    {
                        ...
                        if (targetFXAlignToGround)
                        {
                            ...
                            if (Physics.Raycast(center, Vector3.down, out RaycastHit groundHitInfo,
            ...
            if (useSmoothBlend && _highlighted && somePartVisible)
            {
                ...
                {
                    sourceDesc =
                        new RenderTextureDescriptor(smoothRTWidth,
                            smoothRTHeight); // Application.isMobilePlatform ? RenderTextureFormat.Default : RenderTextureFormat.DefaultHDR);
        ...
        void WorldToViewportPoint(ref Matrix4x4 m, ref Vector4 p, bool perspectiveProjection, float zBufferParamsZ,
        ...
        void ComposeSmoothBlend(Visibility smoothGlowVisibility, Visibility smoothOutlineVisibility)
        {
            ...
            if (renderSmoothGlow)
            {
                fxMatComposeGlow.SetVector(ShaderParams.Flip,
                    (UnityEngine.XR.XRSettings.enabled && flipY) ? new Vector4(1, -1) : new Vector4(0, 1));
                ...
                fxMatComposeGlow.SetInt(ShaderParams.GlowStencilComp,
                    glowIgnoreMask ? (int)CompareFunction.Always : (int)CompareFunction.NotEqual);
        ...
        void ConfigureOutput()
        {
            ...
            var useDepthRenderBuffer = colorAttachmentBuffer != BuiltinRenderTextureType.CameraTarget &&
                                       depthAttachmentBuffer == BuiltinRenderTextureType.CameraTarget;
            cbHighlight.SetRenderTarget(colorAttachmentBuffer,
                useDepthRenderBuffer ? colorAttachmentBuffer : depthAttachmentBuffer);
#endif
        }
        ...
        public void UpdateMaterialProperties()
        {
            ...
            maskRequired = (_highlighted && (outline > 0 || (glow > 0 && !glowIgnoreMask))) ||
            ...
            for (int k = 0; k < rmsCount; k++)
            {
                if (rms[k].mesh != null)
                {
                    ...
                    for (int l = 0; l < rms[k].mesh.subMeshCount; l++)
                    {
                        ...
                        // See-through
                        bool usesSeeThroughBorder = rms[k].fxMatSeeThroughBorder != null &&
